...
def get_dist_all(label_map, train_num, val_num, save_dir, dataset):
    label_num = np.sum(sum(label_map > 0))
    ...
    cate_i_dist[-1] = np.sum(label_map == category_num)
    ...
    for i, [train_i_num, val_i_num, i_num] in enumerate(zip(train_dist, val_dist, cate_i_dist)):
        ...
        val_indices = (i_index[0][shuffle_indices[int(train_i_num):int(train_i_num + val_i_num)]],
                       i_index[1][shuffle_indices[int(train_i_num):int(train_i_num + val_i_num)]])
        test_indices = (i_index[0][shuffle_indices[int(train_i_num + val_i_num):]],
                        i_index[1][shuffle_indices[int(train_i_num + val_i_num):]])
    ...
    plt.title('test_set')
    fig.savefig(str(os.path.join(save_dir, dataset + '_dist_all_train-{}_val-{}.png'.format(train_num, val_num))),
                dpi=100)

    with h5py.File(os.path.join(save_dir, dataset + '_dist_all_train-{}_val-{}.h5'.format(train_num, val_num)),
                   'w') as f:
        f['height'] = m,
        f['width'] = n,
...
def get_dist_per(label_map, train_num, val_num, save_dir, dataset):
    ...
    plt.title('test_set')
    fig.savefig(
        str(os.path.join(save_dir, dataset + '_dist_per_train-{}_val-{}.png'.format(train_per_num, val_per_num))),
        dpi=100)

    with h5py.File(os.path.join(save_dir, dataset + '_dist_per_train-{}_val-{}.h5'.format(train_per_num, val_per_num)),
                   'w') as f:
        f['height'] = m,
        f['width'] = n,
        f['train_label_map'] = train_label_map,
...
def main():
    ...
    parser.add_argument('--dist_dir', type=str, default='./sample')
    parser.add_argument('--dataset', type=str, default='hn3')
    parser.add_argument('--train_num', type=int, default=68000)  #这里输入的是所有类样本的总和
    parser.add_argument('--val_num', type=int, default=8500)     #这里输入的是所有类样本的总和
